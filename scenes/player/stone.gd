...
func _ready() -> void:
	# Standard gravity for flatter alignment with the movement plane
	gravity_scale = 1.0
...
func set_thrower(path: NodePath) -> void:
	...
	if is_instance_valid(_thrower) and is_inside_tree():
		add_collision_exception_with(_thrower)
